...
def run_exploration(init_data, task_desc, persona_desc, webhook_callback=None, stop_event=None):
    try:
        ...
        x, y, width, height = init_data["x"], init_data["y"], init_data["width"], init_data["height"]
        ...
        if webhook_callback and callable(webhook_callback):
            webhook_callback({"status": "completed", "result": result})
        ...
        print_with_color(error_message, "red")
        if webhook_callback and callable(webhook_callback):
            webhook_callback({"status": "error", "message": error_message})
        return {"status": "error", "message": error_message}
